...
export default function ShoeGrid() {
  ...
  const [initialZoom] =
    useState(
      DEFAULT_CONFIG.zoomOut,
    );

  const [currentZoom, setCurrentZoom] =
    useState(rigState.zoom);
  ...
  const [nikeFilter, setNikeFilter] =
    useState("all");
  ...
  const collectionsData =
    useMemo(() => {
      ...
      const newBalanceHalf =
        newBalanceFull.slice(
          0,
          Math.ceil(
            newBalanceFull.length /
              2,
          ),
        );
      ...
      const newBalance = [
        ...
        ...newBalanceHalf.map(
          (shoe, index) => ({
            ...shoe,
            product_url: `${shoe.product_url}-dup-${index}`,
          }),
        ),
      ];
  ...
  const filteredItemCount =
    useMemo(() => {
      if (
        ...
        return activeLayer.items
          .length;
      }

      return activeLayer.items.filter(
        (item) =>
          matchesFilter(
            item,
            nikeFilter,
      ...
      >
        ...
        <fog
          ...
          args={[
            "#F2F1ED",
            controls?.fogNear ??
              DEFAULT_CONFIG.fogNear,
            controls?.fogFar ??
              DEFAULT_CONFIG.fogFar,
          ]}
        />
        ...
        style={{
          ...
          background: `
            linear-gradient(
              180deg,
              transparent 0%,
              rgba(17,17,17,0.012) 55%,
              rgba(17,17,17,0.035) 100%
            )
          `,
        }}
      />
